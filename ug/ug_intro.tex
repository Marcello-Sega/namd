...
%       $Revision: 1.4 $      $Date: 1998/03/06 21:06:31 $
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION:
...
New features in \NAMD\ \NAMDVER\ include periodic boundary conditions, the Berendsen constant pressure simulation method, fixed atoms, several boundary conditions, triple timestepping, full electrostatic evaluation independent of cycle length, and the ability to do on-the-fly analysis and apply forces to the system via Tcl scripts.
The new multiple timestepping parameter \verb!nonbondedFreq! allows nonbonded interactions to be evaluated less frequently, leaving only bonded interactions to be calculated every timestep.
...
Also unimplemented are less-used features such as an applied electric field, force DCD files, and interactive simulation.
...
(DPMTA) \mycite{(Board \ETAL)}{duke:fma}, which takes the full electrostatic
...
\mycite{(Bernstein \ETAL, 1977)}{BERN77}, structure files in PSF format, 
