...
%       $Revision: 1.16 $      $Date: 2001/08/02 14:36:40 $
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION:
...
\subsubsection*{Charmrun on All Platforms}

The former \verb#conv-host# is now \verb#charmrun# and is present on all
...
any platform you can try \verb#charmrun namd2 +p<procs> <configfile>#.

\subsubsection*{Simple Local Usage}

Workstation network versions of NAMD no longer require the ability
to \verb#rsh localhost# or a nodelist file in order to run only on the
local machine.  The new option \verb#++local# spawns processes directly,
as in \verb#charmrun namd2 ++local +p<procs> <configfile>\verb#.

\subsubsection*{Support for Scyld Beowulf}
...
\subsubsection*{Building PSF Structure Files}

A new tool, \verb#psfgen#, has been added, which uses CHARMM topology
files and PDB coordinate files to generate PSF structure files
suitable for use with NAMD.

\subsubsection*{AMBER Input File Compatibility}

NAMD can read AMBER parm (combined structure and parameter) and
coordinate files as input.  Output file formats are still limited
to PDB and CHARMM DCD trajectory files.
