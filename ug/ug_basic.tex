...
cutoff distance, specified by {\tt vdwcutoff} or {\tt cutoff}.  
...
\begin{figure}[htb]
  \center{\psfig{file=figures/switching.eps}}
...
\begin{figure}[htb]
  \center{\psfig{file=figures/shifting.eps}}
...
\begin{figure}[htb]
  \center{\psfig{file=figures/fmaOn.eps}}
...
\begin{figure}[htb]
  \center{\psfig{file=figures/pairlistdist.eps}}
...
between full electrostatic evaluation unless {\tt fmaFrequency} is
also specified.  
It is recommended that {\tt stepspercycle} ({\tt fmaFrequency}) be chosen so that 
...
\item
\NAMDCONFWDEF{nonbondedFreq}{timesteps between nonbonded evaluation}{positive integer factor of {\tt fmaFrequency}}{1}
{This parameter specifies how often short-range nonbonded interactions should be calculated.  Setting {\tt nonbondedFreq} between 1 and {\tt fmaFrequency} allows triple timestepping where, for example, one could evaluate bonded forces every 1 fs, short-range nonbonded forces every 2 fs, and long-range electrostatics every 4 fs.} 
...
{\tt eleccutoff}, {\tt vdwcutoff}, and {\tt cutoff} 
is recommended.  
}
...
\item
\NAMDCONFWDEF{FMAFrequency}{number of timesteps between DPMTA calculations}{positive integer factor of {\tt stepspercycle}}{{\tt stepspercycle}}
...
\item
\NAMDCONFWDEF{MTSAlgorithm}{MTS algorithm to be used}{{\tt verletI}}{{\tt naive}}
...
\item
\NAMDCONFWDEF{longSplitting}{how should long and short range forces be split?}{{\tt xplor}, {\tt c1}}{{\tt sharp}}
