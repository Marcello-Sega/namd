...
% The following two lines moved to ug_add.tex (cbh 2008.09.17)
...
\item %
  \NAMDCONFWDEF{%
    ...
    (global) Colvar values trajectory frequency}{%
    positive integer}{%
    \texttt{1}}{%
...
\item %
  \NAMDCONFWDEF{%
    ...
    the colvars and biasing methods are instructed to perform
    whatever analysis they are configured to.  This is particularly
    useful when the colvar module is compiled as standalone, to
    perform data post--processing for the collective variables without
    restarting NAMD.}
...
%    [TODO - should we keep this option?]
%\item %
%  \NAMDCONFWDEF{%
%    JacobianForce}{%
%    (\texttt{colvar}) Calculate the geometric entropy force}{%
%    boolean}{%
%    \texttt{off}}{%
    % In a few special cases, most notably colvars defined by
    % distance--based components, an alternate formulation of the
    % potential of mean force is traditionally used, which excludes
    % the Jacobian term describing the effect of geometric entropy on
    % the distribution of the variable.  This results, for example, in
    % particle--particle potentials of mean force being flat at large
    % separations.  If all the components of this \texttt{colvar}
    % support it, the corresponding ``Jacobian force'' is calculated,
    % and included in the calculated system force.  This force is
    % based on the
    % gradient of the ``geometric entropy'' arising from a change in
    % the
    % density of states in the coordinate transformation $\{
    % \mathbf{x}_{1}(t), \mathbf{x}_{2}(t) \ldots \mathbf{x}_{N}(t)
    % \}$
    % $\rightarrow$ $\{ \xi(\mathbf{x}_{i}(t)),
    % \mathbf{\tilde{x}}_{1}(\xi, \mathbf{x}_{i}(t)), \ldots \}$.
%    }
...
When the two groups are free to rotate independently, their relative
...
\item %
  \NAMDCONF{%
    atomNumbers}{%
    (atom group) Atoms referred by numbers}{%
    space--separated list of positive integers}{%
    This option adds to the group all the atoms whose (1-based) numbers
    are in the list.}
...
\item %
  \NAMDCONF{%
    ...
    This option adds to the group all the atoms whose (1-based) numbers are
...
\item %
  \NAMDCONF{%
    ...
    \texttt{atomNameResidueRange} within this atom group.
    In the event that atoms from different segments have to be selected,
    it is more convenient to perform the atom selection in VMD, set one of
    the PDB fields to mark the selection and save a PDB file
    to be read with \texttt{atomsFile}.  This option need not be provided
    when non--PSF topologies (e.g. AMBER parmtop) are used by NAMD.}
...
\item %
  \NAMDCONF{%
    ...
    string}{%
    This option adds to the group all atoms having a nonzero
    value in the column \texttt{atomsCol} of the provided PDB file.
    It can be used multiple times for the same group.}
...
\item %
  \NAMDCONFWDEF{%
    ...
    \texttt{X}, \texttt{Y}, \texttt{Z}, \texttt{O} or \texttt{B}}{%
    \texttt{O}}{%
    This option specifies which column in the PDB file is used to
    determine the atoms to be included in the group, for all instances
    of \texttt{atomsFile} within the atom group.}
...
\item %
  \NAMDCONF{%
    ...
    reference position.  If this option is specified, \texttt{disableForces}
    is turned on, the provided position is returned as the
    center of mass, and zero velocities and system forces are
    reported to the colvars.}
...
\item %
  \NAMDCONFWDEF{%
    ...
    \texttt{orientation} colvar component, making use of a quaternion.
    Forces applied to this group from within the colvar module are
    rotated back to the reference frame of the simulation prior being
    communicated to the MD integrator.  \textbf{Note}: care must be
    taken when a torque is not applied on the group, which could
    bring the simulation out of control; to avoid problems,
    \texttt{disableForces} should be used.}
...
\item %
  \NAMDCONF{%
    ...
    UNIX filename}{%
    If either \texttt{centerReference} or \texttt{rotateReference}
    is \texttt{on}, the coordinates from this file are used to
    determine the center of mass's translation and the optimal
    rotation between them and the current coordinates of the group.
    In the latter case, the number of coordinates read from the file
    must be of the same length as this atom group.  This file can
    either contain as many atoms as this group (in which case all
    \texttt{ATOM} records are used) or all the atoms of the
    topology (in which case only the atoms with a non--zero value in
    \texttt{refPositionsCol} are used).  One typical
    usage is to use the file provided as \texttt{atomsFile},
    given that this file contains all the atoms of the group.}
...
\item %
  \NAMDCONFWDEF{%
    ...
    If \texttt{refPositionsFile} is defined, and the file contains
    all the atoms in the topology, this option sets which PDB field is
    used to select the reference coordinates for this group.  If the
    file contains instead only the atoms of this group, this field is
    ignored.}
...
\item %
  \NAMDCONF{%
    rotateRefGroup}{%
    (atom group) Reference group for the rotation}{%
    Block \texttt{rotateRefGroup \{ ... \}}}{%
    If \texttt{centerReference} is defined, this keyword allows to
    allocate another atom group within this one, to act as the
    reference for rotating this group.
    The reference group must have the same number of atoms as
...
\item %
  \NAMDCONFWDEF{%
    ...
    writtein every \texttt{restartFrequency} steps to the file
