...
\item calculate potentials of mean force (PMFs) along any set of colvars, using different sampling methods, such as Adaptive Biasing Force (ABF), metadynamics, steered MD and umbrella sampling, as well as replica-exchange variants of these methods;
...
To enable a colvar calculation, two parameters must be added to the NAMD configuration file: \texttt{colvars} and \texttt{colvarsConfig}; a third parameter, \texttt{colvarsInput} is necessary to restart previous simulation runs.
...
In addition to the NAMD files, the following three output files are written:
\begin{itemize}
...
\item if the NAMD parameter \texttt{restartFreq} is larger than zero, a \emph{restart file} named\\ \texttt{$<$restartName$>$.colvars.state} is written, which is equivalent to the state file; the frequency of writing is every \texttt{restartFreq} steps, or every \texttt{colvarsRestartFrequency} steps if defined; 

\item if the parameter \texttt{colvarsTrajFrequency} is greater than 0 (default: 100), a \emph{trajectory file} is written during the simulation: its name is \texttt{$<$outputName$>$.colvars.traj}; unlike the state file, it is not needed to restart a simulation, but can be used later for post-processing and analysis.
...
\subsubsection{Configuration file for the collective variables and their algorithms}
...
\item %
  \NAMDCONFWDEF{%
    ...
    (global context) Colvar value trajectory frequency}{%
    positive integer}{%
    \texttt{100}}{%
    The instantaneous values of each colvar (and of other derived quantities, if requested) are written to the file \texttt{<outputName>.colvars.traj} every these many steps.
    If the value is \texttt{0}, the trajectory file is not written.
    For optimization, the output is buffered, and synchronized with the disk only every time the restart file is written.}

\item %
  \NAMDCONFWDEF{%
    colvarsTrajAppend}{%
    (global context) Append to trajectory file?}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    ...
    (global context) Colvar module restart frequency}{%
    positive integer}{%
    \texttt{restartFreq}}{%
...
\item %
  \NAMDCONFWDEF{%
    analysis}{%
    (global context) Turn on run-time statistical analysis }{%
    boolean}{%
    \texttt{off}}{%
...
In the following, section \ref{sec:colvar} explains how to define
a colvar and its behavior.  Section \ref{sec:colvar_atom_groups} documents how to select atoms, and section \ref{sec:cvc} lists the available colvar components.
Section \ref{sec:colvarbias} lists the available methods to perform biased
simulations and multidimensional analysis.
...
\item %
  \NAMDCONFWDEF{%
    name}{%
    (\texttt{colvar} context) Name of this colvar}{%
    string}{%
...
\item %
  \NAMDCONFWDEF{%
    width}{%
    (\texttt{colvar} context) Expected fluctuations amplitude, and resolution for grid-based methods}{%
    positive decimal}{%
    1.0}{%
...
\item %
  \NAMDCONF{%
    lowerBoundary}{%
    (\texttt{colvar} context) Lower boundary of the colvar}{%
    decimal}{%
...
\item %
  \NAMDCONF{%
    upperBoundary}{%
    (\texttt{colvar} context) Upper boundary of the colvar}{%
    decimal}{%
...
\item %
  \NAMDCONFWDEF{%
    hardLowerBoundary}{%
    (\texttt{colvar} context) Whether the lower boundary is the physical lower limit}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONF{%
    hardUpperBoundary}{%
    (\texttt{colvar} context) Whether the upper boundary is the physical upper limit of the colvar's values}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    expandBoundaries}{%
    (\texttt{colvar} context) Allow to expand the two boundaries if needed}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONF{%
    lowerWallConstant}{%
    (\texttt{colvar} context) Lower wall force constant (kcal/mol)}{%
    positive decimal}{%
...
\item %
  \NAMDCONFWDEF{%
    lowerWall}{%
    (\texttt{colvar} context) Position of the lower wall}{%
    decimal}{%
    \texttt{lowerBoundary}}{%
...
\item %
  \NAMDCONF{%
    upperWallConstant}{%
    (\texttt{colvar} context) Upper wall force constant (kcal/mol)}{%
    positive decimal}{%
...
\item %
  \NAMDCONFWDEF{%
    upperWall}{%
    (\texttt{colvar} context) Position of the upper wall}{%
    decimal}{%
    \texttt{upperBoundary}}{%
...
\item %
  \NAMDCONFWDEF{%
    outputValue}{%
    (\texttt{colvar} context) Output a trajectory for this colvar}{%
    boolean}{%
    \texttt{on}}{%
...
\item %
  \NAMDCONFWDEF{%
    outputVelocity}{%
    (\texttt{colvar} context) Output a velocity trajectory for this colvar}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    outputEnergy}{%
    (\texttt{colvar} context) Output an energy trajectory for this colvar}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    outputSystemForce}{%
    (\texttt{colvar} context) Output a system force trajectory for this
    colvar}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    outputAppliedForce}{%
    (\texttt{colvar} context) Output an applied force trajectory for this
    colvar}{%
    boolean}{%
    \texttt{off}}{%
...
\paragraph*{Extended Lagrangian.} The following options enable extended-system
...
\item %
  \NAMDCONFWDEF{%
    extendedLagrangian}{%
    (\texttt{colvar} context) Add extended degree of freedom}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    extendedFluctuation}{%
    (\texttt{colvar} context) Standard deviation between the colvar and the fictitious
...
\item %
  \NAMDCONFWDEF{%
    ...
    (\texttt{colvar} context) Oscillation period of the fictitious particle (fs)}{%
    positive decimal}{%
    \texttt{200}}{%
...
\item %
  \NAMDCONFWDEF{%
    extendedTemp}{%
    (\texttt{colvar} context) Temperature for the extended degree of freedom (K)}{%
    positive decimal}{%
...
\item %
  \NAMDCONFWDEF{%
    ...
    (\texttt{colvar} context) Damping factor for extended Langevin dynamics
    (ps$^{-1}$)}{%
    positive decimal}{%
    \texttt{1.0}}{%
...
Atoms that are included by multiple selection keywords will only be counted once.
...
\item %
  \NAMDCONF{%
    atomsCol}{%
    (atom group context) PDB column to use for the selection}{%
...
\item %
  \NAMDCONF{%
    atomsColValue}{%
    (atom group context) Value in the PDB column}{%
    positive decimal}{%
    If defined, this value in \texttt{atomsCol} identifies which records of \texttt{atomsFile} will be read; otherwise, all atoms with a non-zero value in \texttt{atomsCol} will be included in the group.}

\item %
  \NAMDCONF{%
    dummyAtom}{%
    (atom group context) Dummy atom position (\AA{})}{%
    \texttt{(x, y, z)} triplet}{%
...
\paragraph*{Moving frame of reference.} The following options define an automatic calculation of an optimal translation (\texttt{centerReference}) or optimal rotation (\texttt{rotateReference}), that superimposes the positions of this group to a provided set of reference coordinates.
...
\item %
  \NAMDCONF{%
    refPositions}{%
    (atom group context) Reference positions for fitting (\AA)}{%
...
\item %
  \NAMDCONF{%
    ...
    \emph{i)} \texttt{refPositionsCol} is specified: the PDB file contains a set of position larger than the size of the group, and positions will be read according to the value of the column \texttt{refPositionsCol} (which may be the same as \texttt{atomsCol}).
    \emph{ii)} \texttt{refPositionsCol} is not specified and the PDB file contains exactly as many \texttt{ATOM} records as the atoms in the group: all positions will be read in sequence;
    \emph{iii)} \texttt{refPositionsCol} is not specified and the PDB file contains the entire system: the positions corresponding to the numeric indices of the atom group will be read.
}

\item %
  \NAMDCONF{%
    refPositionsCol}{%
    (atom group context) Column to use in the PDB file}{%
    \texttt{X}, \texttt{Y}, \texttt{Z}, \texttt{O} or \texttt{B}}{%
...
\item %
  \NAMDCONFWDEF{%
    enableFitGradients}{%
    (atom group context) Include the roto-translational contribution to colvar gradients}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    enableForces}{%
    (atom group context) Apply forces from this colvar to this group}{%
    boolean}{%
    \texttt{on}}{%
...
\item %
  \NAMDCONFWDEF{%
    corrFunc}{%
    (\texttt{colvar} context) Calculate a time correlation function?}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONF{%
    corrFuncWithColvar}{%
    (\texttt{colvar} context) Colvar name for the correlation function}{%
    string}{%
...
\item%
  \NAMDCONFWDEF{%
    corrFuncType}{%
    (\texttt{colvar} context) Type of the correlation function}{%
...
\item %
  \NAMDCONFWDEF{%
    corrFuncNormalize}{%
    (\texttt{colvar} context) Normalize the time correlation function?}{%
    boolean}{%
    \texttt{on}}{%
...
\item %
  \NAMDCONFWDEF{%
    corrFuncLength}{%
    (\texttt{colvar} context) Length of the time correlation function}{%
    positive integer}{%
    \texttt{1000}}{%
...
\item %
  \NAMDCONFWDEF{%
    corrFuncStride}{%
    (\texttt{colvar} context) Stride of the time correlation function}{%
    positive integer}{%
    \texttt{1}}{%
...
\item %
  \NAMDCONFWDEF{%
    corrFuncOffset}{%
    (\texttt{colvar} context) Offset of the time correlation function}{%
    positive integer}{%
    \texttt{0}}{%
...
\item %
  \NAMDCONFWDEF{%
    corrFuncOutputFile}{%
    (\texttt{colvar} context) Output file for the time correlation function}{%
    UNIX filename}{%
...
\item %
  \NAMDCONFWDEF{%
    runAve}{%
    (\texttt{colvar} context) Calculate the running average and standard deviation}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    runAveLength}{%
    (\texttt{colvar} context) Length of the running average window}{%
    positive integer}{%
    \texttt{1000}}{%
...
\item %
  \NAMDCONFWDEF{%
    runAveStride}{%
    (\texttt{colvar} context) Stride of the running average window values}{%
    positive integer}{%
    \texttt{1}}{%
...
\item %
  \NAMDCONFWDEF{%
    runAveOutputFile}{%
    (\texttt{colvar} context) Output file for the running average and standard deviation}{%
    UNIX filename}{%
...
\item %
  \NAMDCONF{%
    group1}{%
    (\texttt{distance} context) First group of atoms}{%
    Block \texttt{group1 \{...\}}}{%
    First group of atoms.}
\item %
  \NAMDCONF{%
    group2}{%
    (\texttt{distance} context) Second group of atoms}{%
    Block \texttt{group2 \{...\}}}{%
    Second group of atoms.}
\item %
  \NAMDCONFWDEF{%
    forceNoPBC}{%
    (\texttt{distance} context) Calculate absolute rather than minimum-image distance?}{%
    boolean}{%
    \texttt{no}}{%
...
\item %
  \NAMDCONFWDEF{%
    oneSiteSystemForce}{%
    (\texttt{distance} context) Measure system force on group 1 only?}{%
    boolean}{%
    \texttt{no}}{%
...
\item %
  \NAMDCONF{%
    main}{%
    (\texttt{distanceZ, \texttt{distanceXY}} context) Main group of atoms}{%
    Block \texttt{main \{...\}}}{%
    Group of atoms whose position $\bm{r}$ is measured.}
\item %
  \NAMDCONF{%
    ref}{%
    (\texttt{distanceZ, \texttt{distanceXY}} context) Reference group of
    atoms}{%
    Block \texttt{ref \{...\}}}{%
...
\item %
  \NAMDCONFWDEF{%
    ref2}{%
    (\texttt{distanceZ, \texttt{distanceXY}} context) Secondary reference
    group}{%
    Block \texttt{ref2 \{...\}}}{%
    none}{%
...
\item %
  \NAMDCONFWDEF{%
    axis}{%
    (\texttt{distanceZ}, \texttt{distanceXY} context) Projection axis (\AA{})}{%
    \texttt{(x, y, z)} triplet}{%
    \texttt{(0.0, 0.0, 1.0)}}{%
...
\item %
  \NAMDCONFWDEF{%
    forceNoPBC}{%
    (\texttt{distanceZ, distanceXY} context) Calculate absolute rather than minimum-image distance?}{%
    boolean}{%
    \texttt{no}}{%
...
\item %
  \NAMDCONFWDEF{%
    oneSiteSystemForce}{%
    (\texttt{distanceZ, distanceXY} context) Measure system force on group \texttt{main} only?}{%
    boolean}{%
    \texttt{no}}{%
...
\paragraph*{Component \texttt{distanceInv}: mean distance between two groups of atoms.}
...
\item %
  \NAMDCONFWDEF{%
    exponent}{%
    (\texttt{distanceInv} context) Exponent $n$ in equation~\ref{eq:distanceInv}}{%
...
\item \NAMDCONFWDEF{%
    oneSiteSystemForce}{%
    (\texttt{angle}, \texttt{dihedral} context) Measure system force on group 1 only?}{%
    boolean}{%
    \texttt{no}}{%
    If this is set to \texttt{yes}, the system force is measured along
    a vector field (see equation~(\ref{eq:gradient_vector}) in
...
\item %
  \NAMDCONFWDEF{%
    cutoff}{%
    (\texttt{coordNum} context) ``Interaction'' distance (\AA)}{%
    positive decimal}{%
    4.0}{%
...
\item %
  \NAMDCONFWDEF{%
    cutoff3}{%
    (\texttt{coordNum} context) Reference distance vector (\AA)}{%
...
\item %
  \NAMDCONFWDEF{%
    expNumer}{%
    (\texttt{coordNum} context) Numerator exponent}{%
    positive even integer}{%
    6}{%
...
\item %
  \NAMDCONFWDEF{%
    expDenom}{%
    (\texttt{coordNum} context) Denominator exponent}{%
    positive even integer}{%
    12}{%
...
\item %
  \NAMDCONFWDEF{%
    group2CenterOnly}{%
    (\texttt{coordNum} context) Use only \texttt{group2}'s center of
    mass}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONF{%
    refPositions}{%
    (\texttt{rmsd} context) Reference coordinates}{%
    space-separated list of \texttt{(x, y, z)} triplets}{%
...
\item %
  \NAMDCONF{%
    refPositionsFile}{%
    (\texttt{rmsd} context) Reference coordinates file}{%
    UNIX filename}{%
...
\item %
  \NAMDCONF{%
    refPositionsCol}{%
    (\texttt{rmsd} context) PDB column to use}{%
    \texttt{X}, \texttt{Y}, \texttt{Z}, \texttt{O} or \texttt{B}}{%
...
\item %
  \NAMDCONF{%
    refPositionsColValue}{%
    (\texttt{rmsd} context) Value in the PDB column}{%
    positive decimal}{%
...
\item %
  \NAMDCONF{%
    vector}{%
    (\texttt{eigenvector} context) Vector components}{%
    space-separated list of \texttt{(x, y, z)} triplets}{%
...
\item %
  \NAMDCONF{%
    vectorFile}{%
    (\texttt{eigenvector} context) PDB file containing vector components}{%
    UNIX filename}{%
...
\item %
  \NAMDCONF{%
    vectorCol}{%
    (\texttt{eigenvector} context) PDB column used to tag participating atoms}{%
    \texttt{O} or \texttt{B}}{%
...
\item %
  \NAMDCONF{%
    vectorColValue}{%
    (\texttt{eigenvector} context) Value used to tag participating atoms in the PDB file}{%
    positive decimal}{%
...
\item %
  \NAMDCONFWDEF{%
    differenceVector}{%
    (\texttt{eigenvector} context) The $3n$-dimensional vector is the difference between \texttt{vector} and \texttt{refPositions}}{%
    boolean}{%
    \texttt{off}}{%
    If this option is \texttt{on}, the numbers provided by \texttt{vector} or \texttt{vectorFile} are interpreted as another set of positions, $\mathbf{x}_{i}^{\mathrm{(1)}}$: the vector $\mathbf{v}_{i}$ is then defined as $\mathbf{v}_{i} = \left(\mathbf{x}_{i}^{\mathrm{(1)}} - \mathbf{x}_{i}^{\mathrm{(ref)}}\right)$.}
...
\paragraph*{Component \texttt{inertia}: total moment of inertia of a group
  of atoms.}  The block \texttt{inertia~\{...\}} defines the
...
\paragraph*{Component \texttt{inertiaZ}: total moment of inertia of a group
  of atoms around a chosen axis.}  The block \texttt{inertiaZ~\{...\}} defines the
...
\item %
  \NAMDCONFWDEF{%
    axis}{%
    (\texttt{inertiaZ} context) Projection axis (\AA{})}{%
    \texttt{(x, y, z)} triplet}{%
    \texttt{(0.0, 0.0, 1.0)}}{%
...
\item %
  \NAMDCONFWDEF{%
    closestToQuaternion}{%
    (\texttt{orientation} context) Reference rotation}{%
...
\item %
  \NAMDCONF{%
    psfSegID}{%
    (\texttt{alpha} context) PSF segment identifier}{%
...
\item %
  \NAMDCONFWDEF{%
    hBondCoeff}{%
    (\texttt{alpha} context) Coefficient for the hydrogen bond term}{%
...
\item %
  \NAMDCONFWDEF{%
    angleRef}{%
    (\texttt{alpha} context) Reference $\mathrm{C}_{\alpha} -
...
\item %
  \NAMDCONFWDEF{%
    angleTol}{%
    (\texttt{alpha} context) Tolerance in the $\mathrm{C}_{\alpha} -
    \mathrm{C}_{\alpha} - \mathrm{C}_{\alpha}$ angle}{%
    positive decimal}{%
    15$^{\circ}$}{%
...
\item %
  \NAMDCONFWDEF{%
    hBondCutoff}{%
    (\texttt{alpha} context) Hydrogen bond cutoff}{%
    positive decimal}{%
    3.3~\AA{}}{%
...
\item %
  \NAMDCONFWDEF{%
    hBondExpNumer}{%
    (\texttt{alpha} context) Hydrogen bond numerator exponent}{%
    positive integer}{%
    6}{%
...
\item %
  \NAMDCONFWDEF{%
    hBondExpDenom}{%
    (\texttt{alpha} context) Hydrogen bond denominator exponent}{%
    positive integer}{%
    8}{%
...
\item %
  \NAMDCONF{%
    vectorFile}{%
    (\texttt{dihedralPC} context) File containing dihedral PCA eigenvector(s)}{%
    file name}{%
...
\item %
  \NAMDCONF{%
    vectorNumber}{%
    (\texttt{dihedralPC} context) File containing dihedralPCA eigenvector(s)}{%
    positive integer}{%
...
\item %
  \NAMDCONFWDEF{%
    period}{%
    (\texttt{distanceZ} context) Period of the component}{%
    positive decimal}{%
    0.0}{%
...
\item %
  \NAMDCONFWDEF{%
    wrapAround}{%
    (\texttt{distanceZ}, \texttt{dihedral} or \texttt{spinAngle} context) Center
...
\item %
  \NAMDCONFWDEF{%
    name}{%
    (colvar bias context) Identifier for the bias}{%
    string}{%
...
\item %
  \NAMDCONF{%
    colvars}{%
    (colvar bias context) Collective variables involved}{%
...
\begin{itemize}
\item \NAMDCONFWDEF{fullSamples}{(\texttt{abf} context) Number of samples in a bin prior
...
\item \NAMDCONFWDEF{hideJacobian}{(\texttt{abf} context) Remove geometric entropy term from calculated
...
\item \NAMDCONFWDEF{outputFreq}{(\texttt{abf} context) Frequency (in timesteps) at which ABF data files are refreshed}
  {positive integer}
...
\item \NAMDCONFWDEF{historyFreq}{(\texttt{abf} context) Frequency (in timesteps) at which ABF history files are
  accumulated}
  {positive integer}
  {0}
...
\item \NAMDCONF{inputPrefix}{(\texttt{abf} context) Filename prefix for reading ABF data}
  {list of strings}
...
\item \NAMDCONFWDEF{applyBias}{(\texttt{abf} context) Apply the ABF bias?}
  {boolean}
  {\texttt{yes}}
...
\item \NAMDCONFWDEF{updateBias}{(\texttt{abf} context) Update the ABF bias?}
  {boolean}
  {\texttt{yes}}
...
\item %
  \NAMDCONFWDEF{%
    hillWeight}{%
    (\texttt{metadynamics} context) Height of each hill (kcal/mol)}{%
    positive decimal}{%
    \texttt{0.01}}{%
...
\item %
  \NAMDCONFWDEF{%
    newHillFrequency}{%
    (\texttt{metadynamics} context) Frequency of hill creation}{%
    positive integer}{%
    \texttt{100}}{%
...
\item %
  \NAMDCONFWDEF{%
    wellTempered}{%
    (\texttt{metadynamics} context) Perform well-tempered metadynamics}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONF{%
    biasTemperature}{%
    (\texttt{metadynamics} context) Temperature bias for well-tempered     metadynamics}{%
    positive decimal}{%
...
\item %
  \NAMDCONFWDEF{%
    useGrids}{%
    (\texttt{metadynamics} context) Interpolate the hills with grids}{%
    boolean}{%
    \texttt{on}}{%
...
\item %
  \NAMDCONFWDEF{%
    hillWidth}{%
    (\texttt{metadynamics} context) Relative width of the hills}{%
    positive decimal}{%
    $\sqrt{2\pi}/2$}{%
...
\item %
  \NAMDCONFWDEF{%
    dumpFreeEnergyFile}{%
    (\texttt{metadynamics} context) Periodically write the PMF for visualization}{%
    boolean}{%
    \texttt{on}}{%
...
\item %
  \NAMDCONFWDEF{%
    rebinGrids}{%
    (\texttt{metadynamics} context) Recompute the grids when reading a state
    file}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    multipleReplicas}{%
    (\texttt{metadynamics} context) Multiple replicas metadynamics}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONF{%
    replicaID}{%
    (\texttt{metadynamics} context) Set the identifier for this replica}{%
    string}{%
...
\item %
  \NAMDCONFWDEF{%
    replicasRegistry}{%
    (\texttt{metadynamics} context) Multiple replicas database file}{%
    UNIX filename}{%
...
\item %
  \NAMDCONFWDEF{%
    ...
    (\texttt{metadynamics} context) How often hills are communicated between
    replicas}{%
    positive integer}{%
...
\item %
  \NAMDCONFWDEF{%
    ...
    (\texttt{metadynamics} context) Periodically write the contribution to the
...
\item %
  \NAMDCONFWDEF{%
    name}{%
    (\texttt{metadynamics} context) Name of this metadynamics instance}{%
    string}{%
...
%     (\texttt{metadynamics} context) Frequency of update of the grids}{%
%     positive integer}{%
%     \texttt{newHillFrequency}}{%
...
\item %
  \NAMDCONFWDEF{%
    saveFreeEnergyFile}{%
    (\texttt{metadynamics} context) Keep all the PMF files}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    keepHills}{%
    (\texttt{metadynamics} context) Write each individual hill to the state
    file}{%
    boolean}{%
    \texttt{off}}{%
...
\item %
  \NAMDCONFWDEF{%
    ...
    (\texttt{metadynamics} context) Write a log of new hills}{%
    boolean}{%
    \texttt{on}}{%
...
\item %
  \NAMDCONFWDEF{%
    forceConstant}{%
    (\texttt{harmonic} context) Scaled force constant (kcal/mol)}{%
    positive decimal}{%
    \texttt{1.0}}{%
...
\item %
  \NAMDCONF{%
    centers}{%
    (\texttt{harmonic} context) Initial harmonic restraint centers}{%
...
\item %
  \NAMDCONF{%
    targetCenters}{%
    (\texttt{harmonic} context) Steer the restraint centers towards these
    targets}{%
    space-separated list of colvar values}{%
...
\item %
  \NAMDCONF{%
    targetForceConstant}{%
    (\texttt{harmonic} context) Change the force constant towards this value}{%
    positive decimal}{%
...
\item %
  \NAMDCONF{%
    targetNumSteps}{%
    (\texttt{harmonic} context) Number of steps for steering}{%
    positive integer}{%
...
\item %
  \NAMDCONF{%
    targetEquilSteps}{%
    (\texttt{harmonic} context) Number of steps discarded from TI estimate}{%
    positive integer}{%
...
\item %
  \NAMDCONFWDEF{%
    targetNumStages}{%
    (\texttt{harmonic} context) Number of stages for steering}{%
...
\item %
  \NAMDCONF{%
    lambdaSchedule}{%
    (\texttt{harmonic} context) Schedule of lambda-points for changing force constant}{%
...
\begin{itemize}
\item \NAMDCONFWDEF{outputFreq}{(\texttt{histogram} context) Frequency (in timesteps) at which the histogram file is refreshed}
  {positive integer}
  {Colvar module restart frequency}
