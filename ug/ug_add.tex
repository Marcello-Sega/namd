...
Locally enhanced sampling (LES) increases sampling and transition rates
...
{positive integer equal to or less than the number of images present}
{This should generally be equal to the factor used in {\tt multiply}
 when creating the structure.  The nonbonded potential of images is
 divided by {\tt lesFactor}.  If atoms are present with an {\tt lesCol}
 value in {\tt lesFile} that is smaller than {\tt lesFactor}, then the
...
\item
\NAMDCONFWDEF{lesCol}{column of PDB file containing LES flags}{{\tt X}, {\tt Y}, {\tt Z}, {\tt O}, or {\tt B}}{{\tt O}}
...
{\em Note that the default column is O, while \PSFGEN\ writes this value
in column B, so this parameter is normally required.}}
