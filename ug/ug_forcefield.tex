...
We presently recommend using the default MSMQuality 0 
($C^1$ cubic interpolation with $C^2$ Taylor splitting)
which has been validated to correctly reproduce the PME results
for water properties and
for air--water and membrane--water interfaces~\cite{HARD2015}. 
Although these parameters for MSM give slower performance 
than for $C^1$ Hermite interpolation, 
roughly half the speed, 
there appears to be a bug with Hermite interpolation 
which, in particular, does not give correct results 
for the air--water interface. 
Once the optimizations described in Ref.~\cite{HARD2015} 
have been completed in \NAMD, 
we expect the performance for cubic interpolation 
to be comparable to Hermite interpolation. 
