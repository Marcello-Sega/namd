...
static char rcsid[]="$Id: dpmta_mastiface.c,v 1.3 1997/09/29 23:58:37 jim Exp $";

/*
 * revision history:
 *
 * $Log: dpmta_mastiface.c,v $
 * Revision 1.3  1997/09/29 23:58:37  jim
 ...
 *
 * Revision 1.1  1997/09/05 19:41:53  jim
...
#ifndef PIPED
static PmtaVector CubeLength;      /* size of edge of simulation space */
#else
static PmtaVector PVector1;
static PmtaVector PVector2;
static PmtaVector PVector3;
#endif
static PmtaVector CubeCenter;      /* center coordinates of simulation space */
...
int PMTAinit(PmtaInitDataPtr initdata, int *rtn_tids)
{
   ...
   /* these will have to be put as parameters */
   int startlevel = 1;
...
#ifndef PIPED
   CubeLength.x = initdata->v1.x;
   CubeLength.y = initdata->v2.y;
   CubeLength.z = initdata->v3.z;
#else
   PVector1.x = initdata->v1.x;
   PVector1.y = initdata->v1.y;
   PVector1.z = initdata->v1.z;
   PVector2.x = initdata->v2.x;
   PVector2.y = initdata->v2.y;
   PVector2.z = initdata->v2.z;
   PVector3.x = initdata->v3.x;
   PVector3.y = initdata->v3.y;
   PVector3.z = initdata->v3.z;
#endif
   CubeCenter.x = initdata->cellctr.x;
   CubeCenter.y = initdata->cellctr.y;
   CubeCenter.z = initdata->cellctr.z;
   Resize = TRUE;
...
#ifndef PIPED
   pvm_upkdouble(&(CubeLength.x),3,1);
#else
   pvm_upkdouble(&(PVector1.x),3,1);
   pvm_upkdouble(&(PVector2.x),3,1);
   pvm_upkdouble(&(PVector3.x),3,1);
#endif
   pvm_upkdouble(&(CubeCenter.x),3,1);
   Resize = TRUE;
...
#ifndef PIPED
      Nlevels, Resize, &(CubeLength), &(CubeCenter), MyProcNum );
#else
      Nlevels, Resize, &(PVector1), &(PVector2), &(PVector3),
      &(CubeCenter), MyProcNum );
#endif
...
#ifdef TIME
   Recv_Slave_Times(MyProcNum,Nprocs,Tids);
#endif
...
int PMTAresize( pvector1, pvector2, pvector3, cellctr)
   PmtaVector *pvector1, *pvector2, *pvector3;
   PmtaVector *cellctr;   /* center of simulation cube */

{
#ifndef PIPED
   CubeLength.x = pvector1->x;
   CubeLength.y = pvector2->y;
   CubeLength.z = pvector3->z;
#else
   PVector1.x = pvector1->x;
   PVector1.y = pvector1->y;
   PVector1.z = pvector1->z;
   PVector2.x = pvector2->x;
   PVector2.y = pvector2->y;
   PVector2.z = pvector2->z;
   PVector3.x = pvector3->x;
   PVector3.y = pvector3->y;
   PVector3.z = pvector3->z;
#endif
   CubeCenter.x = cellctr->x;
   CubeCenter.y = cellctr->y;
   CubeCenter.z = cellctr->z;
   Resize = TRUE;
...
*
*  kills off all slaves processes.
*
