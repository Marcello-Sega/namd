...
static char rcsid[] = "$Id: dpmta_slvmkiil.c,v 1.1 1997/09/05 19:42:04 jim Exp $";
...
sort_ii_list()
{
   int i,j,k,l;
   int level, cell, proc;
   int tcell, pcell, lcell, index, mask;
   int temp1, temp2;
   int ovfl;
   ...
   for (level=Dpmta_DownPassStart; level<Dpmta_NumLevels; level++) {
      if ( Dpmta_Scell[level] != -1 ) {
         for (cell=Dpmta_Scell[level]; cell<=Dpmta_Ecell[level]; cell++) {
...
dump_ii_list()
{
   ...
   nlongs = Dpmta_LevelLocate[Dpmta_NumLevels]/32 + 1;
   for (i=0; i < Dpmta_Nproc; i++) {
      fprintf(fp,"Proc %d:\n",i);
