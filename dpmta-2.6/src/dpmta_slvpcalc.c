...
static char rcsid[] = "$Id: dpmta_slvpcalc.c,v 1.1 1997/09/05 19:42:07 jim Exp $";

/*
 * revision history:
 *
 * $Log: dpmta_slvpcalc.c,v $
 * Revision 1.1  1997/09/05 19:42:07  jim
...
Slave_Direct_Calc()
{
   ...
   for ( i=Dpmta_Scell[level]; i<=Dpmta_Ecell[level]; i++) {
      ...
      for ( j=0; j<Dpmta_Intlist[posn].dcnt; j++ ) {
         ...
         if ( Cell2Cell(level,i,sep,&rcell,&ovfl) ) {
	    ...
	    */
 	       ...
 	       if ( rcell > i ) {
                  Cell_Calc_DDirect(level,i,level,rcell,
	    ...
	    */
	       ...
	       Cell_Calc_SDirect(level,i,level,rcell,
				 &(Dpmta_Hlist[posn].dlist_vec[j]));
	    } /* else not pid */
...
*  particles within the remote cell.  the contents of the remote
*  cell are not updated.
*
*/
