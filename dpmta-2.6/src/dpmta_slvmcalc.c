...
int Slave_Mpole_Exp()
{
   int i,j,k,l;                /* loop counters, misc indexes */
   int level;
   int done;
   ...
   */
   ...
   while (done == FALSE) {
      ...
      *  Scell and Ecell array for the parent level will not
      *  be sufficient.
      */
      ...
      */
      ...
      level--;
      if ( Dpmta_Scell[level] == -1 )
	 done = TRUE;
#ifndef SERIAL
      else
	 Rcv_Mpe_from_Child(level);
#endif
...
int Slave_MPE_Calc()
{
   ...
   double *scratch;              /* used for initialization */
...
int Calc_multipole_exp(level,cell)
int level;
int cell;
   {
...
int FFT_Multipole(level,cell)
int cell;
   {
   Mtype mpole;

   if ( Dpmta_CellTbl[level][cell]->mvalid == FALSE )
      return;
   mpole = Dpmta_CellTbl[level][cell]->m;
...
Calc_M2M(clevel, ccell, plevel, pcell)
int clevel;       /* child level */
int ccell;        /* child cell */
int plevel;       /* parent level */
int pcell;        /* parent cell */
...
Calc_L2L(plevel, pcell, clevel, ccell)
int plevel;       /* parent level */
int pcell;        /* parent cell */
int clevel;       /* child level */
int ccell;        /* child cell */
   ...
   */
   ...
   if ( Dpmta_CellTbl[plevel][pcell]->mdata->lvalid == FALSE ) {
      return;
   } /* if lvalid */
   ...
   */
   ...
   v.x = Dpmta_CellTbl[clevel][ccell]->p.x - Dpmta_CellTbl[plevel][pcell]->p.x;
   v.y = Dpmta_CellTbl[clevel][ccell]->p.y - Dpmta_CellTbl[plevel][pcell]->p.y;
   v.z = Dpmta_CellTbl[clevel][ccell]->p.z - Dpmta_CellTbl[plevel][pcell]->p.z;
...
#ifdef COMP_LJ
   , MtypeLJ h_lj     /* LJ mpe transfer matrix */
#endif
   )
   ...
   )
{
   ...
   if ( Dpmta_CellTbl[llevel][lcell]->mdata->lvalid == FALSE )
      return;
...

{
   ...
   */
   ...
   if ( Dpmta_CellTbl[level][cell]->mdata->lvalid == FALSE )
      return;
...
#ifdef TIME
   times(&endbuf);
   times_arr[14] = (double)(endbuf.tms_utime - startbuf.tms_utime) / (double)CLK_TCK;
#endif
