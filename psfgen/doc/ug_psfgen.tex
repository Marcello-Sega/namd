...
\item \COMMAND{\IKEY{topology} \ARG{file name}}
...
\item \COMMAND{\IKEY{segment} \ARG{segment ID} \{ \ARG{commands} \}}
...
\item \COMMAND{\IKEY{residue} \ARG{resid} \ARG{resname}}
...
\item \COMMAND{\IKEY{patch} \ARG{patch residue name} \ARG{segid:resid} \OARG{...}}
...
context is destroyed and ``deleted \ARG{old context}'' is returned.  An error is
returned if the specified context does not exist or if \KEY{delete} was
specified and the specified context is the same as the current context.}
{\ARGDEF{context}{Context ID returned by psfcontext.}}
{At any time.}

\item \COMMAND{\IKEY{writepsf} \OKEY{charmm} \OKEY{x-plor} \ARG{file name}}
