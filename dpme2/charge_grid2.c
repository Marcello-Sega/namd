...
/* --------------------------------------------------------------------- */
  int theta1_dim1, theta1_offset, theta2_dim1, theta2_offset, 
    ...
    /* part 2 loop over the rest of atoms */
    nn=nlocal; /* use in indexing arrays created w/dveotor ie fr123 */
    ncg=0; /* index the charge array */
    for (n = nlocal+1; n <= (*numatoms); ++n,++nn,++ncg) {
      k0 = (int)(fr3[nn]) - *order;
      for (ith3 = 1; ith3 <= (*order); ++ith3) {
	++k0;
	k = k0 + 1 + (*nfft3 - Nsign(*nfft3, k0)) / 2;
	...
	for (ith2 = 1; ith2 <= (*order); ++ith2) {
	  ...
	  j = j0 + 1 + (*nfft2 - Nsign(*nfft2, j0)) / 2;
	  prod = theta2[ith2 + n * theta2_dim1] * 
	    theta3[ith3 + n * theta3_dim1] * tmpcg[ncg];
	  i0 = (int)fr1[nn] - *order;
	  for (ith1 = 1; ith1 <=( *order); ++ith1) {
	    ++i0;
	    i = i0 + 1 + (*nfft1 - Nsign(*nfft1, i0)) / 2;
...
#endif
   /* PART 1 , loop over my local particles */
    nn=atompnt; /* use in indexing arrays created w/dvecotor ie fr123 */
...
/* ----------------------------------------------------------- */
/* Subroutine */ int grad_sum( int *nlocal,int atompnt, int *list, 
			      Pme2Particle *ParticlePtr, 
