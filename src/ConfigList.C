...
//// Osman Sarood
//// Parallel Input
// Helper function to pack ConfigList object to be passed in messages
char* ConfigList::packHelper(ConfigListMessage *inmsg)
{
  int totalSize=0;
  totalSize+=sizeof(int)*2;

  ConfigListNode *ptr=inmsg->cfgList->theList;
  int count=0;
  while(ptr!=NULL)
  {
    count++;
  ...
  while(ptr!=NULL)
  {
    totalSize+=sizeof(int)+strlen(ptr->name);
    StringList *inPtr=ptr->data;
    int inCount=0;
    while(inPtr!=NULL)
    {
      inCount++;
      totalSize+=sizeof(int)+strlen(inPtr->data);
      inPtr=inPtr->next;
    }
    numDataFields[count]=inCount;
  ...
  memcpy((buf+sizeof(int)),&(inmsg->numAtoms),sizeof(int));
  memcpy((buf+sizeof(int)*2),&(inmsg->doFlip),sizeof(bool));
  memcpy(&readCount,buf,sizeof(int));


  count=0;
  ptr=inmsg->cfgList->theList;
  int writeStart=sizeof(int)*2+sizeof(bool);
  while(ptr!=NULL)
  {
//printf("name;;%s\n",ptr->name);
    int nameLen=-1;
    if(ptr->name!=NULL) nameLen=strlen(ptr->name);
    memcpy((void*)(buf+writeStart),&nameLen,sizeof(int));
    writeStart+=sizeof(int);
    memcpy((void*)(buf+writeStart),ptr->name,strlen(ptr->name));
    writeStart+=strlen(ptr->name);
    memcpy((void*)(buf+writeStart),&numDataFields[count],sizeof(int));
    writeStart+=sizeof(int);
    StringList *inPtr=ptr->data;
    int inCount=0;
    while(inPtr!=NULL)
    {
      int dataLen=strlen(inPtr->data);
      memcpy((void*)(buf+writeStart),&dataLen,sizeof(int));
      writeStart+=sizeof(int);
      memcpy((void*)(buf+writeStart),inPtr->data,strlen(inPtr->data));
      writeStart+=strlen(inPtr->data);
      inPtr=inPtr->next;
    }
    count++;
    ptr=ptr->next;
