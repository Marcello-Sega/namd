 ...
 * 
 ***************************************************************************/
static char ident[] = "@(#)$Header: /home/cvs/namd/cvsroot/namd2/src/PDBData.C,v 1.1 1996/08/06 20:38:38 ari Exp $";
...
void PDBAtom::parse_column_data( const char *data)
{
     ...
     // set the residue sequence
 {
    ...
    if (s[0] < '0' || s[0] > '9') {
       ...
       if (s[0] < 'A' || s[0] > 'Z') {
	  if (elvis_count == 0) {
fprintf(stderr, "Man, tiny Elvis, that number is huge!\n");
fprintf(stderr, "We don't know how X-Plor represents over Z999 residues\n");
fprintf(stderr, "And you just tried %s - so we'll fake it as %d\n", s, num);
fprintf(stderr, "This is reversible, but only inside this program.\n");
               elvis_count = 1;
	   }
       } else {
	  elvis_count = 0;
...
PDBData *new_PDBData(const char *data)  // nasty
{
  ...
  for (int i=0; i<sizeof(PDBData::PDBNames) /
