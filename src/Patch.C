...
/*
// DMK
#if defined(NAMD_MIC) && MIC_SUBMIT_ATOMS_ON_ARRIVAL != 0

// DMK - NOTE: May wish to move this functionality into ComputeNonbondedMICKernel.h/C, but for now
//   the association is with the patches (i.e. buffer per patch), so leave here for now as we test
//   to see if this actually helps performance or not.
void Patch::submitMICAtomData(int deviceNum, int doneMigrationFlag) {

  atom* &hostPtr = atomData_hostPtr.item(deviceNum);
  uint64_t &devicePtr = atomData_devicePtr.item(deviceNum);
  int &allocSize = atomData_allocSize.item(deviceNum);

  // If the allocated size is already larger than the requested size, nothing to do so just return
  //   NOTE: The number of atoms should only resize while the doneMigrationFlag is also set
  int numAtoms_16 = ((numAtoms + 15) & (~15));
  if (hostPtr == NULL || allocSize < requiredSize) {

    // NOTE: The body of this if statement should correspond to an (increasingly) rare events, so
    //   doesn't have to be greatly optimized
    // Increase the required size a bit to reduce the frequency of reallocations
    allocSize = (int)((float)requiredSize * 1.2f);
    allocSize = ((allocSize + 15) & (~15));  // but keep allocate size a multiple of 16

    // Free the current device and host memory
    #pragma offload target(mic:deviceNum) \
      nocopy(hostPtr[0:0] : alloc_if(0) free_if(1))
    { }
    _mm_free(hostPtr);

    // Allocate the new memory
    hostPtr = _mm_malloc(sizeof(atom) * requiredSize * 2, MIC_ALIGN);
    __ASSERT(hostPtr != NULL);
    #pragma offload target(mic:deviceNum) \
      in(hostPtr[0:allocSize*2] : alloc_if(1) free_if(0) align(MIC_ALIGN)) \
      out(devicePtr)
    { devicePtr = (uint64_t)hostPtr; }

  } // end if (atomData_allocSize < requiredSize)


  // If atoms have just migrated, push the required atom_param data
  if (doneMigrationFlag) {

    // Copy the portions of the extended atom data needed on the device
    CompAtom *a = p.begin();
    ComputeAtomExt *aExt = pExt.begin();
    atom_param *atom_params = (atom_param*)(hostPtr + numAtoms_16);
    #if MIC_HANDCODE_FORCE_SOA_VS_AOS != 0
    #else
      int *atom_params_vdwType     = ((int*)atom_params) + (0 * numAtoms_16);
      int *atom_params_index       = ((int*)atom_params) + (1 * numAtoms_16);
      int *atom_params_exclIndex   = ((int*)atom_params) + (2 * numAtoms_16);
      int *atom_params_exclMaxDiff = ((int*)atom_params) + (3 * numAtoms_16);
      for (int k = 0; k < numAtoms; k++) {
        int j = aExt[k].sortOrder;
        atom_params_vdwType[k] = a[j].vdwType;
        atom_params_index[k] = aExt[j].id;
        #ifdef MEM_OPT_VERSION
          atom_params_exclIndex[k] = exclusionsByAtom[aExt[j].exclId].y;

          // DMK - Continue here... the issue is that exclusionsByAtom is located within ComputeNonbondedMIC.
          //   In general, there needs to be a buffer associated with each patch, making the patch files ideal
          //   for declaring this work, but this also places MIC code outside of the MIC files.  Also, the
          //   data needed is, at least partially, MIC specific and in those files.  Figure out what to do
          //   with this stuff...

      }
    #endif

  } // end if (doneMigrationFlag)
