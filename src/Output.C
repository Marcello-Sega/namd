...
void Output::recv_vmd_patch_loads()
{
  ...
  int i;        //  Loop counter
  int num_patches;    //  Number of patches in the current message
  ...
  while (notReceived)
  {
    msg = comm->newInputStream(ANY, PATCHLOADTAG);
    msg->get(num_patches);

    //  If there is any real load information, get it
    if (num_patches)
    {
      patches = new int[num_patches];
      loads = new Real[num_patches];
      atomsPerPatch = new int[num_patches];
       msg->get(num_patches, patches);
       msg->get(num_patches, loads);
       msg->get(num_patches, atomsPerPatch);
 ...
 *  $Revision: 1.16 $  $Date: 1998/10/24 19:57:45 $
 *
 ***************************************************************************
 * REVISION HISTORY:
 *
