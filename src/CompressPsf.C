...
int operator==(const AtomSigInfo &s1, const AtomSigInfo& s2){
    ...
    if(equalCnt!=dihedralSigCnt) return 0;
    ...
    if(equalCnt!=improperSigCnt) return 0;
...
int operator==(const ExclSigInfo &s1, const ExclSigInfo &s2){
	...
	for(int i=0; i<s1.fullExclOffset.size(); i++){
		if(s1.fullExclOffset[i] != s2.fullExclOffset[i]) return 0;
	}
	
	for(int i=0; i<s1.modExclOffset.size(); i++){
		if(s1.modExclOffset[i] != s2.modExclOffset[i]) return 0;
	}
	return 1;
}
...
void getBondData(FILE *fd){
  ...
  while (num_read < numBonds)
  {
    ...
    if(atom_nums[0] == atom_nums[1]){ //bond to itself
        char err_msg[128];
  ...
  for(int i=0; i<numBonds; i++){
      ...
      if(!newSig){//check duplicate bonds in the form of (a, b) && (a, b);          
...
void getAngleData(FILE *fd){
    ...
    Angle *angles=new Angle[numAngles];
...
void getDihedralData(FILE *fd){
  ...
  Dihedral *dihedrals = new Dihedral[numDihedrals];
